...
while True:
    ...
    # Run detection with tracking using YOLOv8's built-in tracker
    results = model.track(frame, persist=True, tracker="botsort.yaml")  # or "bytetrack.yaml"
    ...
    if results[0].boxes.id is not None:
        ...
        tracked_objects = zip(boxes, track_ids, confs, class_ids)
    ...
    print(f"\nFrame-{frame_number}")
    for box, track_id, conf, class_id in tracked_objects:
        if class_id != 0:  # Skip non-person objects
            continue
    ...
    for box, track_id, conf, class_id in tracked_objects:
        if class_id == 0:  # Skip person objects
            continue
    ...
    annotated_frame = results[0].plot()  # This will draw all detections with tracking IDs
    cv2.imshow("Classroom Tracking", annotated_frame)
